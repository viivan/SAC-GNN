...
import MAIGNet_no_att
import MAIGNet_no_gate
import pop_test
...
# 多重训练
def multi_train(mashup_list, api_list, pos_sample_num, neg_sample_num, m_a_id, model, optimizer, train_sim_dic, scheduler=None):
    ...
    elif args.loss_func == 'sample_sim_bpr':
        # 根据相似度信息，计算mean
        # 通过mashup_features和sim_service来求对应的test_features
        train_sim_feature_list = []  # test_num个
        for t_i in mashup_list:
            # 把对应的结果mean-pooling
            single_sim_train_index = train_sim_dic[t_i]
            single_sim_train_features = mashup_embeddings[single_sim_train_index]  # cold_sim_num个
            ...
            train_sim_feature_list.append(single_mean_test_feature)
        train_sim_features = torch.cat(train_sim_feature_list, dim=0)

        # 算两回相加
        loss_0 = model.multi_bpr_loss(mashup_list, mashup_embeddings, api_embeddings, m_pos_dic, m_neg_dic)
        loss_1 = model.multi_bpr_loss(mashup_list, train_sim_features, api_embeddings, m_pos_dic, m_neg_dic)
        loss = loss_0 + loss_1
...
if __name__ == "__main__":
    ...
    # 把pop较小的去除
    pop_dict = pop_test.cal_pop_dict(m_id, a_id, m_a_id)
    ...
    MAIG_adj_matrix, MAIG_norm_adj_matrix = gc.create_scipy_adjacency_matrix_MAIG(m_id, a_id, train_m_a_id)
    ...
    # 找到train中的相似mashup
    single_train_sim_service_dic = None
    single_train_sim_train_service_dic = None
    if args.loss_func == 'sample_sim_bpr':
        single_train_sim_service_dic, single_train_sim_train_service_dic = c_s.cal_sim_service_from_train(train_mashup_list, train_mashup_list,
                                                                            t_m_bert_feature, args.cold_sim_num)
    ...
    schedule = StepLR(optimizer, step_size=1000, gamma=0.3)
    ...
    for i in range(args.epoch):
        ...
        loss = multi_train(train_mashup_list, a_id, args.pos_sample_num, args.neg_sample_num, m_a_id, model, optimizer, single_train_sim_train_service_dic,
                           schedule)
        t2 = time.time()
