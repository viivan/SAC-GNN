...
if __name__ == "__main__":
    ...
    mashup_train_file = "recommend_mashup_api_id_train%_0.2_2.txt"
    mashup_test_file = "recommend_mashup_api_id_test%_0.2_2.txt"
    train_m_id, train_a_id, train_m_a_id = dr.read_mashup_api(mashup_train_file)
    test_m_id, test_a_id, test_m_a_id = dr.read_mashup_api(mashup_test_file)

    # 为了test能对应上，test_m_a_id进行填充工作
    whole_test_m_a_id = []
    for i in m_id:
        if i in test_m_id:
            place = test_m_id.index(i)
            whole_test_m_a_id.append(test_m_a_id[place])
        else:
            whole_test_m_a_id.append([])
    ...
    schedule = StepLR(optimizer, step_size=1000, gamma=0.1)
    ...
    for i in range(args.epoch):
        ...
        loss = multi_train(m_id, a_id, args.pos_sample_num, args.neg_sample_num, train_m_a_id, model, optimizer,
                           schedule)
        t2 = time.time()
        ...
        if i != 0 and i % 20 == 0:
            ...
            t5 = time.time()
            print("train metric:")
            model_test.test(model, m_id, a_id, train_m_a_id, args)
            t6 = time.time()
            print("time {}".format(round(t6 - t5, 1)))

            t3 = time.time()
            print("test metric:")
            average_hrk, average_ndcg = model_test.test_for_test(model, test_m_id, a_id,  whole_test_m_a_id, train_m_a_id, args)
